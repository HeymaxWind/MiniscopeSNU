function aviFileList = msCamVideoFileDetection(path, filePrefix)

aviFiles = dir([char(path) '\*.avi']); % 경로에 모든 avi file을 찾고

cnt1 = 0; 
for j = 1:length(aviFiles)
    if aviFiles(j).name(1:length(filePrefix)) == filePrefix  % 검색된 동영상 앞부분이 filePrefix에 해당하면
